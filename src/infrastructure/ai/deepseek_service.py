    ...
    async def analyze_study_stats(
        ...
        SYSTEM_PROMPT = f"""Ты — высококлассный ( но очень дружелюбный ) специалист по анализу образовательных данных и когнитивный психолог в приложении для интервального повторения.
             ...
             Имя пользователя: {input_data.user_name}
               ...
               1. **Инсайты (insights)** Можешь указать тут хорошие метрики! Сделать прогнозы по будущему времени занятий и как скоро освоит материал ( если это разбор для одной колоды а не общий ) Так же если в контексте есть переданные цели с прошлого разбора то обязательно разбери их тут:
