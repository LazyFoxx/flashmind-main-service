...
from src.application.exceptions import UserAlreadyExistsError, UserNotFoundError
        ...
        async with self.uow:
            try:
                ...
                if not existing_user:
                    ...
                    raise UserNotFoundError(f"Пользователь {input_dto.user_id} не найден")
            
                updates = {}
                if input_dto.first_name is not None:  # Проверяем на None, чтобы не перезаписывать пустыми
                ...
                if input_dto.avatar_file:
                    new_avatar_key = await self.storage.put_avatar(
                        input_dto.user_id, category="avatar", file=input_dto.avatar_file
                    )
                    old_key = existing_user.avatar_key
                    if old_key:
                        await self.storage.delete_object(object_key=old_key, user_id=existing_user.id)
                    updates["avatar_key"] = new_avatar_key


                updated_user = dataclasses.replace(existing_user, **updates)
                await self.uow.users.update(user=updated_user)
                await self.uow.commit()
                self.logger.info(
                    "Профиль пользователя успешно обнавлен в БД", user_id=updated_user.id
                )

                existing_user = updated_user  # Перезапишем для дальнейшего использования
