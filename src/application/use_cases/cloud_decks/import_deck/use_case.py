...
from src.application.exceptions import DeckNotExistsError, DeckImportFromOwnAuthorError, CloudDeckNotExistsError
...
class ImportDeckUseCase:
    ...
    async def execute(self, input_dto: ImportDeckInput) -> ImportDeckOutput:
        ...
        except (CloudDeckNotExistsError, DeckImportFromOwnAuthorError):
             # Перебрасываем уже известные ошибки
            raise
        except Exception as e:
            self.logger.error(
                "Ошибка при импорте колоды",
                error=str(e),
                cloud_uuid=input_dto.cloud_uuid,
                user_id=input_dto.user_id,
            )
